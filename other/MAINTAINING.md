...
When we receive a pull request, a CircleCI build is kicked off automatically (see
the `.circleci.yml` for what runs in the CircleCI build). We avoid merging anything
that breaks the CircleCI build.
...
CircleCI build gets kicked off and if it's successful, a tool called
